...
class collator_class:
    def __init__(self, epoch, step, dataset, batch_size=1):
        self.current_epoch = epoch
        self.current_step = step
    ...
    def __call__(self, examples):
        ...
        # For batch_size > 1, we need to combine multiple examples
        # Each example is a dict with keys like "images", "latents", "input_ids", etc.
        if not examples:
        ...
        for key in examples[0].keys():
            if key in ["images", "latents"]:
                # Stack tensors for images and latents
                combined_batch[key] = torch.stack([ex[key] for ex in examples if key in ex])
            elif key in ["input_ids", "attention_mask"]:
                # Pad sequences for text inputs
                max_len = max(len(ex[key]) for ex in examples if key in ex)
                padded_sequences = []
                for ex in examples:
                    if key in ex:
                        seq = ex[key]
                        if len(seq) < max_len:
                            ...
                            padded = torch.cat([seq, torch.zeros(max_len - len(seq), dtype=seq.dtype, device=seq.device)])
                        else:
                            padded = seq
                        padded_sequences.append(padded)
                    else:
                        # Create zero tensor if key doesn't exist
                        padded_sequences.append(torch.zeros(max_len, dtype=torch.long))
                combined_batch[key] = torch.stack(padded_sequences)
            else:
                # For other keys, just take the first example's value
                # This might need adjustment based on specific use cases
                combined_batch[key] = examples[0][key]
